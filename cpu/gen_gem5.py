...
def make_conf_multi_core(core_types, cache_types, freqs, num_cores):
	...
	return ("_".join([gem5_conf.get_core(core)[0] for core in core_types])+"Core_"+"_".join([gem5_conf.get_cache(cache_type)[0] for cache_type in cache_types])+"Cache_"+"_".join([gem5_conf.get_freq(freq)[0] for freq in freqs]),
			gem5_conf.build_conf(sim_type, env_gem5_bin, env_gem5_src, core_types, cache_types, num_cores, freqs, kernel_image,disk_image,mem_size, sys_clock))
...
def gen_conf_files(con_list):
	...
	benchname = basename(rcs_src).split(".")[0]
	arch_dir = basename(config_path)
	sh_dest = join(config_path, benchname + ".sh")
	...
	with open(sh_dest, "w+") as job_sh:
		...
		job_sh.write("{run_gem5} {rcs} {cpt_dir} 1>>{rcs_stdout} 2>>{rcs_stderr}".format(
			...
			rcs_stdout = join(output_path, benchname + ".out"),
			rcs_stderr = join(output_path, benchname + ".err")
			));
		job_sh.write("\n")
...
def build_script_gem5():
	...
	for conf in range(0,len(gem5_confs)):
		...
		for rcs in tmp_list:
			...
			config_list_multi.append([config_path, config_file, gem5_script, rcs_src])
